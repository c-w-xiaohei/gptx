...
All command examples use `--dry-run --json` first. For the real call, remove `--dry-run` after checking planned paths, and add `--bg` for normal or long image API runs so the session can continue while the job completes. Use `gptx job wait <job_id>` when the next step depends on the saved output. Use repeatable `--context <path>` for text briefs, copy, and requirements. SVG is not a supported `--context` or `--image` input; rasterize SVG logos to PNG/WebP before using `--image`. Do not combine `--dry-run` and `--bg`.
